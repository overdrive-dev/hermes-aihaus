...
9. **Review Dev** — after Dev promotion is verified, Hermes moves the issue to `Review Dev` and stops automation. `Review Dev` is a human review gate.
10. **Box Dev Features** — after human approval, the user/human reviewer moves the issue to `Box Dev Features` and applies a release/box code such as `#01` or `#02` as metadata (label/cycle), not as a new workflow state per release by default.
11. **Linear closeout** — Hermes comments status, blockers, evidence, follow-up links, and final result back to Linear throughout the workflow, not only at the end.
