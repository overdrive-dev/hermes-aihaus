...
12. After Dev promotion is verified, move the issue to `Review Dev` and stop automation for that issue. `Review Dev` is a human review gate; Hermes/agents must not auto-approve it. If the human reviewer approves, the human/user moves the issue to `Box Dev Features` and applies the release/box code as metadata such as label or cycle `#01`, `#02`, etc.
13. Comment evidence back to Linear immediately after each stage transition or gate result; Linear is the operational source of truth, not a final after-action summary.
...
- `Review Dev`: human gate after successful Dev promotion. Automation stops here.
